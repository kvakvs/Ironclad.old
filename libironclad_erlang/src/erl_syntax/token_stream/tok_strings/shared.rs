...
pub(crate) fn parse_escaped_whitespace<'a>(input: TokInput<'a>) -> TokResult<TokInput<'a>> {
  preceded(char('\\'), multispace1)(input)
}

/// `take_while_m_n` parses between `m` and `n` bytes (inclusive) that match
/// a predicate. `parse_hex` here parses between 1 and 6 hexadecimal numerals.
fn parse_hex(inp0: TokInput) -> TokResult<TokInput> {
  take_while_m_n(1, 6, |c: char| c.is_ascii_hexdigit())(inp0)
}

/// `preceded` takes a prefix parser, and if it succeeds, returns the result
/// of the body parser. In this case, it parses u{XXXX}.
fn parse_delimited_hex(input: TokInput) -> TokResult<TokInput> {
  preceded(
    char('u'),
...
// TODO: Bignums!
pub(crate) fn parse_u32(input: TokInput) -> TokResult<u32> {
  map_res(parse_delimited_hex, move |hex: TokInput| u32::from_str_radix(hex, 16))(input)
}
...
pub(crate) fn parse_escaped_char(input: TokInput) -> TokResult<Char> {
  preceded(
    char('\\'),
