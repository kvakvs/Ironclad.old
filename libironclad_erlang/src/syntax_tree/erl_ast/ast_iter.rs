...
impl AstNode for ErlAst {
  ...
  fn children(&self) -> Option<Vec<Arc<ErlAst>>> {
    match self {
      ...
      ErlAst::BinaryOp { expr: binop_expr, .. } => Some(vec![binop_expr.left.clone(), binop_expr.right.clone()]),
      ...
      ErlAst::ListComprehensionGenerator { left, right, .. } => Some(vec![left.clone(), right.clone()]),
