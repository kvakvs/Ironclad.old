...
impl ErlParser {
  ...
  fn parse_expr_prec_primary(input: &str) -> AstParserResult {
  ...
  fn parse_expr_prec02(input: &str) -> AstParserResult {
    Self::parse_expr_prec01(input)
  }

  /// Precedence 3: Unary + - bnot not
  fn parse_expr_prec03(input: &str) -> AstParserResult {
    combinator::map(
      sequence::pair(
      ...
      sequence::pair(
        ...
        multi::many0(
          sequence::pair(
            ...
            Self::ws_before(Self::parse_expr_prec03),
          )
        ),
      ),
  ...
  fn parse_expr_prec05(input: &str) -> AstParserResult {
    combinator::map(
      // Higher precedence expr, followed by 0 or more operators and higher prec exprs
      sequence::tuple((
        Self::parse_expr_prec04,
        multi::many0(
          sequence::pair(
            Self::ws_before_mut(branch::alt((
            ...
            Self::ws_before(Self::parse_expr_prec04),
          )
        )
      )),
      |(left, tail)| ErlBinaryOperatorExpr::new_left_assoc(&SourceLoc::None, left, &tail),
  ...
  fn parse_expr_prec06(input: &str) -> AstParserResult {
    combinator::map(
      // Higher precedence expr, followed by 0 or more operators and higher prec exprs
      sequence::pair(
        Self::parse_expr_prec05,
        multi::many0(
          sequence::pair(
            Self::ws_before_mut(branch::alt((
              Self::binop_list_append, Self::binop_list_subtract,
            ))),
            Self::ws_before(Self::parse_expr_prec05),
          )
        ),
      ),
  ...
  /// Precedence 7: == /= =< < >= > =:= =/=
  fn parse_expr_prec07(input: &str) -> AstParserResult {
    combinator::map(
      // Higher precedence expr, followed by 0 or more operators and higher prec exprs
      sequence::pair(
        Self::parse_expr_prec06,
        multi::many0(
          sequence::pair(
            Self::ws_before_mut(branch::alt((
            ...
            Self::ws_before(Self::parse_expr_prec06),
          )
        ),
      ),
      |(left, tail)| ErlBinaryOperatorExpr::new_left_assoc(&SourceLoc::None, left, &tail),
  ...
  fn parse_expr_prec13(input: &str) -> AstParserResult {
    combinator::map(
      // Higher precedence expr, followed by 0 or more binary operators and higher prec exprs
      sequence::pair(
        Self::ws_before(Self::parse_expr_prec11),
        multi::many0(
          sequence::pair(
            Self::ws_before_mut(branch::alt((
