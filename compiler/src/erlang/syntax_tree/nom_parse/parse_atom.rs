      ...
      E: error::ParseError<&'a str> + error::FromExternalError<&'a str, std::num::ParseIntError>,
{
  branch::alt((
    ...
    // of that parser.
    combinator::map(parse_literal, StringFragment::Literal),
    combinator::map(parse_escaped_char, StringFragment::EscapedChar),
    combinator::value(StringFragment::EscapedWS, parse_escaped_whitespace),
  ))(input)
}
